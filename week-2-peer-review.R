...
#dat <- drop_na(read_csv("homework2_cces_sample_coursera.csv"))
...
dat2<-dat%>%select("educ", "edloan","employ", "CC18_308a")
dat2
...
#### Write your code below:
trump_approve_disapprove<-recode(dat2$CC18_308a,`1`=1,`2`=1,`3`=0,`4`=0)

dat2$trump_approve_disapprove <- trump_approve_disapprove
dat2
...
#### Write your code below:
dat %>% filter(employ==1 & marstat==1) %>% summarise("mean"=mean(pew_religimp), "median"=median(pew_religimp))
