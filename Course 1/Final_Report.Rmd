...
subset<-filter(dat, region %in% c(1,2) & gender==1 & marstat==1 & newsint==1)
str(subset)
```
...
cc18325a=recode(dat$CC18_325a, `1`=1, `2`=0)
cc18325d=recode(dat$CC18_325d, `1`=1, `2`=0)
res_support=cc18325a+cc18325d
dat=mutate(dat, res_support)
dat$res_support[1:20]
...
```{r problem9}
dat%>%group_by(region)%>%summarise(Approval_mean=mean(CC18_308a))
```
...
```{r problem10}
dat%>%filter(investor==2,faminc_new%in% seq(5:10))%>%summarise(mean=mean(pew_religimp), median=median(pew_religimp), sd=sd(pew_religimp))
```
...
```{r problem11}
kable(summarise(dat, mean=mean(faminc_new), median=median(faminc_new), sd=sd(faminc_new)))
```
...
qplot(dat$pid7, geom="histogram", xlab="Seven Point Party ID", ylab="count")
```